<script lang="ts">
	...
	const processStakeData = (data: Array<PredictionMarketStakeEvent>) => {
		...
		const yesData = data.reduce((acc: Array<number>, event, index) => {
			const cumulative = (acc[index - 1] || 0) + (event.index === 1 ? Number(fmtMicroToStx(event.amount, sip10Data.decimals)) : 0);
			acc.push(cumulative);
			return acc;
		}, []);

		const noData = data.reduce((acc: Array<number>, event, index) => {
			const cumulative = (acc[index - 1] || 0) + (event.index === 0 ? Number(fmtMicroToStx(event.amount, sip10Data.decimals)) : 0);
			acc.push(cumulative);
			return acc;
		}, []);
