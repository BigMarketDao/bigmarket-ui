<script lang="ts">
	...
	function generateRoot() {
		if (contractIds && contractIds.length > 0) {
			const { tree, root } = generateMerkleTreeUsingStandardPrincipal(contractIds);
...
<div class="mx-auto px-4 text-white shadow-md">
	...
	<div class="mb-6">
		...
		<button on:click={addFromTextarea} class="mt-2 rounded bg-blue-500 px-4 py-2 text-sm hover:bg-blue-600 focus:outline-none"> Add Contracts </button>
	</div>
	...
	<div class="mb-6">
		...
		<select id="popular-contracts" on:change={(e) => addContract(e)} class="h-10 w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
		...
		<ul class="list-inside list-disc">
			{#each contractIds as id}
				<li class="mb-2 flex items-center justify-between">
					...
					<button on:click={() => removeContract(id)} class="text-red-500 hover:text-red-700 text-lg font-bold focus:outline-none"> &times; </button>
				</li>
			{/each}
		</ul>
	</div>
