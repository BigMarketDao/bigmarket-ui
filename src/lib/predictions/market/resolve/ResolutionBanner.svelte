...
{#if market.resolutionState !== ResolutionState.RESOLUTION_OPEN}
	<div class="">
		<div
			class={market.outcome
				? 'bg-green-100 flex items-start  rounded-md p-4 font-medium'
				: ' flex items-start rounded-md bg-yellow-200 p-4 font-medium'}
		>
			<svg
			...
			<div class="flex flex-col">
				...
				{:else if market.resolutionState === ResolutionState.RESOLUTION_RESOLVED}
					<p class="ml-4 text-sm text-yellow-800">
						Market is resolved. The outcome is <span class="font-medium"
							>{market.outcome ? 'YES' : 'NO'}</span
						>.
					</p>
				{/if}
			</div>
