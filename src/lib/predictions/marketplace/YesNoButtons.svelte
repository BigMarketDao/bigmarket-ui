<script lang="ts">
	...
	import { calculatePayoutCategorical, convertFiatToNative, getMarketToken, type Payout } from '../predictions';
...
{#if market}
	...
	{#if payouts}
		<div class="my-2 flex w-full justify-between text-sm">
			<div class="w-full text-center font-inter font-medium text-black">{$selectedCurrency.symbol}100 -&gt; {payouts[0].fiat}</div>
			<div class="w-full text-center font-inter font-medium text-black">{$selectedCurrency.symbol}100 -&gt; {payouts[1].fiat}</div>
		</div>
	{/if}
{/if}
